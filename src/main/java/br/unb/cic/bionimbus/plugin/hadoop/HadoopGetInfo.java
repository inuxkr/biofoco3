...
public class HadoopGetInfo implements Callable<PluginInfo> {
	...
	public static void getTaskInfo(PluginTask task) throws Exception {
		...
		//System.out.println("Buscando informações " + task.getJobInfo().getLocalId());
		...
		while ((s = br.readLine()) != null) {
			Pattern pattern = Pattern.compile("<td>(\\d+\\D\\d+)%<table", Pattern.CASE_INSENSITIVE);
			Matcher matcher = pattern.matcher(s);

			while (matcher.find()) {
				//System.out.println("Buscando Percentual:");
				double percent = Double.parseDouble(matcher.group(1).replaceAll(",", "."));
				
				//System.out.println(percent);
				// DEBUG
				// System.out.println(percent);
				if (percent > 0.0) {
					//task.getJobInfo().setStartTimestamp(System.currentTimeMillis());
